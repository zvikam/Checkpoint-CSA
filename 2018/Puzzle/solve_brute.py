    ...
    def turn_to_match_frame(self):
        while not self.check_match_frame(False):
            if not self.get_cube().rotate():
                break
        self.check_match_frame(False)

    # def turn_to_match(self):
    #     while not self.check_match():
    #         self.get_cube().rotate()
    #     self.check_match()
...
class Board(object):
    ...
    def solve(self, i):
        for c in range(i, len(self._cells)):
            while self._cells[c].try_next():
                if self.solve(c+1) or self.check():
                    return True
        return False
